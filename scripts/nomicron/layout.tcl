...
proc build_bordered_container {} {
    global theme
    # Outer bordered container that will hold both logo and main content
    frame .bordered_container -bg $theme(border) -relief raised -borderwidth 3
    pack .bordered_container -side top -fill both -expand 1 -padx 8 -pady 8

    # Inner frame for content
    frame .bordered_container.inner -bg $theme(base)
    pack .bordered_container.inner -fill both -expand 1 -padx 2 -pady 2
}
...
proc build_tome_image {} {
    ...
    if {[file exists $tome_path]} {
        ...
        # Pack image inside the bordered container
        frame .bordered_container.inner.image_section -bg $theme(base)
        label .bordered_container.inner.image_section.image -image tome_image -bg $theme(base)
        pack .bordered_container.inner.image_section.image -pady 8
        pack .bordered_container.inner.image_section -side top -fill x
    }
}
...
proc build_main_layout {} {
    ...
    frame .bordered_container.inner.main_container -bg $theme(base)
    pack .bordered_container.inner.main_container -side top -fill both -expand 1 -padx 4 -pady 4

    # Create alias for easier access
    set ::main_container .bordered_container.inner.main_container
